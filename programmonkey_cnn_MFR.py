...
def main(train_file, validation_file):
	...
	x_train=change_mrf(x_train)
	x_validation=change_mrf(x_validation)
...
def change_mrf(image_set):
	im=np.zeros((image_set.shape[0],image_set.shape[1],image_set.shape[2],image_set.shape[3]))

	for num in range(image_set.shape[0]):
		image=image_set[num]
		#print(image.shape)
		for chanel in range(3):
			image_one_chanel=image[:,:,chanel]
			
			#print(image_one_chanel.shape)
			mid_image=scipy.ndimage.median_filter(image_one_chanel,size=(3,3))
			im[num,:,:,chanel]=mid_image-image_one_chanel
